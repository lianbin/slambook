				 ...
				 const Mat &inputs, const Mat &outputs, Mat params)
{
    ...
    Mat input(1, n, CV_64F); // single row input  //单一输入

    double last_mse = 0;

    for(int i=0; i < MAX_ITER; i++) {//最大迭代次数
        double mse = 0;
        for(int j=0; j < m; j++) {//残差方程的个数
            ...
            mse += r.at<double>(j,0)*r.at<double>(j,0);//残差的平方
            //对每个残差方程，求关于所有参数的雅克比
