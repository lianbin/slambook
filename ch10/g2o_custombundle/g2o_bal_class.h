...
public:
    ...
    {
        T predictions[2];
        CamProjectionWithDistortion ( camera, point, predictions );//注意这里计算投影以及残差的方式，
        residuals[0] = predictions[0] - T ( measurement() ( 0 ) ); //具体是由相机模型决定的，这里的优化
        residuals[1] = predictions[1] - T ( measurement() ( 1 ) );// 使用的数据集以及相机模型见
