...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
    ...
    const double* raw_cameras = bal_problem->cameras();//位姿在内存中的存储地址
    for(int i = 0; i < num_cameras; ++i)//位姿个数
    {
    ...
    // Set edges for graph..
    const int  num_observations = bal_problem->num_observations();//观测 
    ...
    for(int i = 0; i < num_observations; ++i)
    {
        ...
        if(params.robustify)
        {
            ...
            rk->setDelta(1.0);
            bal_edge->setRobustKernel(rk); //鲁棒核函数
        }
        ...
        bal_edge->setInformation(Eigen::Matrix2d::Identity()); //设置信息矩阵为单位阵
...
{   
    ...
    optimizer->setAlgorithm(solver); //设置优化器算法
}
